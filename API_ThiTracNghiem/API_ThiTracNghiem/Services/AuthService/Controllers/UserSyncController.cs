...
namespace API_ThiTracNghiem.Services.AuthService.Controllers
{
    ...
    public class UserSyncController : ControllerBase
    {
        ...
        /// <summary>
        /// Lấy danh sách tất cả Admin users (RoleId = 1)
        /// </summary>
        [HttpGet("admins")]
        public async Task<ActionResult<List<UserSyncDto>>> GetAdminUsers()
        {
            try
            {
                var adminUsers = await _db.Users
                    .Include(u => u.Role)
                    .Where(u => u.RoleId == 1 && !u.HasDelete && u.Status == "active")
                    .Select(u => new UserSyncDto
                    {
                        UserId = u.UserId,
                        Email = u.Email,
                        FullName = u.FullName,
                        RoleId = u.RoleId,
                        RoleName = u.Role != null ? u.Role.RoleName : "Admin",
                        Status = u.Status,
                        IsEmailVerified = u.IsEmailVerified,
                        CreatedAt = u.CreatedAt,
                        UpdatedAt = u.UpdatedAt,
                ...
                return Ok(new { success = true, data = adminUsers, count = adminUsers.Count });
            }
            catch (Exception ex)
            {
