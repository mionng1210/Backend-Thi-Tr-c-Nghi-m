        ...
        public async Task<IActionResult> CreateRoom([FromBody] CreateRoomRequest request)
        {
            try
            {
                ...
                if (string.Equals(request.RoomType, "Support", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var adminUsers = await _userSyncService.GetAdminUsersAsync();
                    
                    _logger.LogInformation($"Found {adminUsers.Count} admin user(s) from AuthService");

                    foreach (var admin in adminUsers)
                    {
                        // Only add if not already a member (creator)
                        if (admin.UserId != creatorId)
                        {
                            // Sync admin to ChatService DB if not exists
                            var existingAdmin = await _context.Users.FindAsync(admin.UserId);
                            if (existingAdmin == null)
                            {
                                _logger.LogInformation($"Syncing admin user {admin.UserId} ({admin.Email}) to ChatService DB");
                            ...
                            // Add admin to room
                            var adminMembership = new ChatRoomMember
                            {
                                RoomId = room.RoomId,
                                UserId = admin.UserId,
                                Role = "member",
                            ...
                            _context.ChatRoomMembers.Add(adminMembership);
                            
                            _logger.LogInformation($"Added admin {admin.UserId} ({admin.Email}) to Support room {room.RoomId}");
                        }
                    }
                }
