...
namespace ChatService.Services
{
    ...
    public interface IUserSyncService
    {
        ...
        Task<List<UserSyncDto>> GetAdminUsersAsync();
    }
    ...
    public class UserSyncService : IUserSyncService
    {
        ...
        public async Task<UserSyncDto?> GetUserByIdAsync(int userId)
        {
            try
            {
                ...
                var response = await _httpClient.GetAsync($"{authServiceUrl}/api/UserSync/user/{userId}");
        ...
        /// <summary>
        /// Lấy danh sách tất cả Admin users từ AuthService
        /// </summary>
        public async Task<List<UserSyncDto>> GetAdminUsersAsync()
        {
            try
            {
                var authServiceUrl = _configuration["Services:AuthService:BaseUrl"];
                ...
                {
                    _logger.LogError("AuthService BaseUrl not configured");
                    return new List<UserSyncDto>();
                }

                var response = await _httpClient.GetAsync($"{authServiceUrl}/api/UserSync/admins");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        var admins = JsonSerializer.Deserialize<List<UserSyncDto>>(dataElement.GetRawText(), new JsonSerializerOptions
                        {
                            PropertyNameCaseInsensitive = true
                        });
                        
                        _logger.LogInformation($"Successfully fetched {admins?.Count ?? 0} admin users from AuthService");
                        return admins ?? new List<UserSyncDto>();
                    }
                }

                _logger.LogWarning($"Failed to get admin users from AuthService: {response.StatusCode}");
                return new List<UserSyncDto>();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting admin users from AuthService");
                return new List<UserSyncDto>();
            }
        }
    }
