...
namespace API_ThiTracNghiem.Middleware
{
    ...
    public class UserSyncMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, IUserSyncService userSyncService, ExamsDbContext dbContext)
        {
            ...
            if (!string.IsNullOrEmpty(authHeader) && authHeader.StartsWith("Bearer "))
            {
                try
                {
                    ...
                    if (user != null)
                    {
                        // ✅ Kiểm tra và sync user vào database nếu chưa tồn tại (fix Foreign Key constraint)
                        ...
                        if (existingUser == null)
                        {
                            ...
                            _logger.LogInformation($"✅ User {user.UserId} ({user.Email}) synced to ExamsService database");
                        }
