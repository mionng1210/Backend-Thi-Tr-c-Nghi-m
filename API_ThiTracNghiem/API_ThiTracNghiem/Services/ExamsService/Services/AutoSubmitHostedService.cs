        ...
        private async Task ProcessExpiredAttemptsAsync(ExamsDbContext db, IExamProgressCache cache, CancellationToken ct)
        {
            ...
            foreach (var examAttempt in expiredAttempts)
            {
                try
                {
                    ...
                    Dictionary<int, AttemptAnswerCache> cached;
                    try
                    {
                        cached = await cache.GetAllAsync(examAttempt.ExamAttemptId);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "Cache unavailable for attempt {AttemptId}, proceeding without answers", examAttempt.ExamAttemptId);
                        cached = new Dictionary<int, AttemptAnswerCache>();
                    }
                    ...
                    {
                        _logger.LogWarning(ex, "Failed to delete cache for attempt {AttemptId}", examAttempt.ExamAttemptId);
                    }
                }
                catch (Exception ex)
                {
